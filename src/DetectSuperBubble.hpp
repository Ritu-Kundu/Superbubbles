...
namespace supbub {
     ...
     *  - 4.Filtering out 'unreal' superbubbles. (Conversion to acyclic requires duplication of parts of the subgraph which may result into some superbubbles('unreal') detected that occur in parts which do not belong to the original subgraph G.)
     * Steps 1, 2 and 4 are described in the paper(paper-1) "An O(m log m) -time Algorithm For Detecting Superbubbles" by Sung et al.
     * Step 3 is implemented as described in the paper (paper-2) "Linear-Time Superbubble Identification Algorithm for  Genome Assembly" by Brankovic et.al.
     *
     * This class provides for the following:
     *  - Finding superbubbles in a general directed graph.
     */
    ...
    class DetectSuperBubble {
      public:
        ...
        struct SuperBubble {
            VertexID_t entrance; // id of entrance vertex
            VertexID_t exit; // id of exit vertex
        };

        /** type definition for list of superbubbles */
        typedef std::list<SuperBubble> supperbubbleList_t;
         ...
         */
        void find( Graph &g, supperbubbleList_t &superBubblesList );
      ...
      private:
         ...
         */
        void superBubble( DAG *dag, VertexID_t *superBubblesArray );
         ...
         * @param superBubblesArray pointer to the array of superbubbles in which result is to be returned.
         */
        void reportSuperBubble( DAG *dag, uint64_t *mark, Candidate *start, Candidate *exit, VertexID_t *superBubblesArray );
         ...
         */
        Candidate * validateSuperBubble( DAG *dag, Candidate *startCand, Candidate *endCand );
