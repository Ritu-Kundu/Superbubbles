...
namespace supbub {
    void DetectSuperBubble::find( Graph &g, supperbubbleList_t &superBubblesList ) {
        uint64_t numVertices = g.numVertices();

        /*******************************STAGE 1***************************************/

        /* Find scc (all singleton-vertices seen as scc with id zero, non-singleton starts from 1) */
        uint64_t *scc = new uint64_t[numVertices];
        uint64_t numSubgraphs = g.fillSCC( scc );

        /* Preprocess before partitioning */
        std::vector<Subgraph *> subgraphs;
        uint64_t *globalToLocalIdMap = new VertexID_t[numVertices]; // Keep track of local-ids given to vertices in each subgraph

        uint64_t *sizeSubgraph = new uint64_t[numSubgraphs]; // Keep track of size of each subgraph
        ...
        for( VertexID_t v = 0; v < numVertices; ++v ) {
            ...
            VertexID_List_t &children = g.getChildren( v );
            ...
            // add edges r-v
            VertexID_List_t &parents = g.getParents( v );
            if( !parents.empty()) {
        ...
        uint64_t DAGSize = sg0->numVertices();
        DAG *dag0 = new DAG( DAGSize );

        for( VertexID_t v = 0; v < DAGSize; ++v ) {
            VertexID_List_t &children = sg0->getChildren( v );
            if( !children.empty()) { // out-degree non-zero
                for( i = children.begin(); i != children.end(); ++i ) {
        ...
        VertexID_t *superBubblesArray = new VertexID_t[DAGSize];
        std::fill_n( superBubblesArray, DAGSize, -1 ); // set to -1
        // find superbubles
        superBubble( dag0, superBubblesArray );

        // Filter out 'unreal' superbubbles
        VertexID_t lastPossibleS = DAGSize - 2; // (last two vertices are dummu source and sink)
        VertexID_t dumyTerminal = dag0->getTerminalId();
        for( VertexID_t s = 0; s < lastPossibleS; ++s ) {
            VertexID_t t = superBubblesArray[ s ];
        ...
        for( uint64_t i = 1; i < numSubgraphs; ++i ) {
            ...
            superBubblesArray = new VertexID_t[dag->numVertices()];
            std::fill_n( superBubblesArray, dag->numVertices(), -1 ); // set to -1

            superBubble( dag, superBubblesArray );

            // Filter out 'unreal' superbubbles
            VertexID_t lastPossibleS = sg->getOffset();
            VertexID_t dumyTerminal = dag->getTerminalId();
            for( VertexID_t s = 0; s < lastPossibleS; ++s ) { // we will only consider u' and not u'' or r' or r''
                VertexID_t t = superBubblesArray[ s ];
                if( t != -1 && t != dumyTerminal ) { // s is entrance of possible 'real' superbubble

                    if( sg->isDuplicateId( t )) { // It corresponds to <s', t''>
                        VertexID_t realT = sg->getOriginalId( t );
                        if( sg->isAncestor( realT, s )) { // if t is ancestor of s, <s, t> is superbubble
                            superBubblesList.push_back( SuperBubble{ sg->getGlobalId( s ), sg->getGlobalId( realT ) } );

                        }
                    }
                    else { // it corresponds to <s', t'>
                        VertexID_t s2 = sg->getDuplicateId( s );
                        VertexID_t t2 = sg->getDuplicateId( t );
    ...
    void DetectSuperBubble::superBubble( DAG *dag, VertexID_t *superBubblesArray ) {
         ...
         */
        uint64_t *mark = new uint64_t[dag->numVertices()];
    ...
    void DetectSuperBubble::reportSuperBubble( DAG *dag, uint64_t *mark, Candidate *start, Candidate *exit, VertexID_t *superBubblesArray ) {
        // sanity check
        ...
        uint64_t exitVer = exit->vertexId;
        dag->candidates.delete_tail();
    ...
    Candidate *DetectSuperBubble::validateSuperBubble( DAG *dag, Candidate *startCand, Candidate *endCand ) {
        uint64_t start = dag->ordD[ startCand->vertexId ];
        uint64_t end = dag->ordD[ endCand->vertexId ];
        uint64_t outChild = dag->rangeMaxOutChild( start, end - 1 );
        uint64_t outParent = dag->rangeMinOutParent( start + 1, end );
